...
if user_input:
    ...
    if has_executable_intent(st.session_state.chat_attachments):
        sys_parts.append(
            ...
            "   이 헬퍼는 중복 이름을 `_2`, `_3` 으로 자동 dedupe — 뒤에서 rename 금지. "
            "**컨텍스트에 표시된 `cols_multi` 와 정확히 같은 이름이 나온다.**\n"
            "\n"
            ...
            "— pandas 2.x 에서 제거돼 `AttributeError: 'OpenpyxlWriter' object has no attribute 'save'` "
            "가 난다. 다중 시트 + 다단 헤더 저장 패턴은 다음과 같다:\n"
            "   ```python\n"
            "   with pd.ExcelWriter('result.xlsx', engine='openpyxl') as w:\n"
            "       final.to_excel(w, sheet_name='Sheet1', index=False)\n"
            "       # 여기서 w.save() 호출 금지! with 종료 시 자동 저장됨.\n"
            "   ```\n"
            "\n"
            ...
            "평탄화한 컬럼명(`계획예산`, `실행예산_이월예산`, `실행예산_당해예산`, …)을 다시 "
            "두 단으로 복원해 `pd.MultiIndex.from_tuples` 로 컬럼 설정 후 저장:\n"
            "   ```python\n"
            ...
            "   result_df.to_excel('result.xlsx', engine='openpyxl')  # MultiIndex 자동 처리\n"
            "   ```\n"
            "\n"
            "   **(b) 카테고리 그룹 + 그룹별 소계 행 보존** — 원본에 `비목분류` 같은 상위 "
            "카테고리가 있었다면 결과도 동일 그룹 순서를 유지하고, 각 그룹 끝에 **그룹 소계** "
            "행을 명시적으로 추가:\n"
            "   ```python\n"
            "   parts = []\n"
            ...
            "   ```\n"
            "\n"
            "   **(c) 컬럼 순서 유지** — 원본 컬럼 순서를 result 에서도 그대로. 사용자가 명시 "
            "요청하지 않는 한 정렬·재배치 금지.\n"
            "\n"
            "   **(d) openpyxl 후처리 (가능하면)** — column width auto-fit + 숫자 천 단위 콤마:\n"
            "   ```python\n"
            ...
            "   ```\n"
            "\n"
            "   원본에 다단 헤더·그룹 구조가 *없는* 단순 표라면 (a)(b) 는 건너뛰고 단일 헤더로 "
            "충분하다. 핵심은 **사용자가 원본 엑셀을 다시 받았을 때 '같은 양식으로 정리된 새 "
            "파일' 처럼 느끼게 하는 것**.\n"
            "\n"
